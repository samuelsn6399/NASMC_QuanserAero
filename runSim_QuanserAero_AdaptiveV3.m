...
%% Configure Plots
plot_type = 1; % 0=general viewing; 1=report (larger font)
if plot_type==0
    line_width = 0.75;
    text_size = 16;
    legend_size = 16;
...
hold off
title('$\hat{h}$ Parameter','Interpreter','Latex',FontSize=title_size)
xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{h}$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
box on
...
hold off
title('$\hat{a}_1$ Parameter','Interpreter','Latex',FontSize=title_size)
xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{a}_1$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
box on
...
hold off
title('$\hat{a}_2$ Parameter','Interpreter','Latex',FontSize=title_size)
xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{a}_2$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
box on
...
hold off
title('$\hat{a}_3$ Parameter','Interpreter','Latex',FontSize=title_size)
xlabel('Time (s)','Interpreter','Latex','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{a}_3$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
box on
...
hold off
title('$\hat{a}_4$ Parameter','Interpreter','Latex',FontSize=title_size)
xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{a}_4$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
box on
...
%% Estimated Params Over Time (Combined)
figure('Name','EstimateParams')
sgtitle("Adaptation Params Over Time",'Interpreter','Latex',FontSize=sgtitle_size)
% Pitch: Column 1
subplot(5,2,1)
hold on
plot(time,haaaa_hat_p(:,1),'b','LineWidth',line_width,'DisplayName','Adaptation')
% plot(time,ones(size(time)).*paramsTruth_haaaa(1,1),'b--','LineWidth',line_width,'DisplayName','Truth')
hold off
title('Pitch Parameters','Interpreter','Latex',FontSize=title_size)
% xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
ylabel('$\hat{h}$','Interpreter','Latex',FontSize=axis_size)
% legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
...
% plot(time,ones(size(time)).*paramsTruth_haaaa(1,2),'b--','LineWidth',line_width,'DisplayName','Truth')
hold off
title('Yaw Parameters','Interpreter','Latex',FontSize=title_size)
% xlabel('Time (s)','Interpreter','Latex',FontSize=axis_size)
% ylabel('$\hat{h}$','Interpreter','Latex',FontSize=axis_size)
legend('Interpreter','Latex','Location','best',FontSize=legend_size)
grid on
...
%% Estimated Parameters Over Time (Combined)
figure('Name','EstimateParams','Color','w','Position',[100 100 900 900])
tiledlayout(5,2,'TileSpacing','compact','Padding','compact');
% tiledlayout(5,2,'TileSpacing','compact');
sgtitle('\textbf{Adaptation Parameters Over Time}','Interpreter','latex','FontSize',sgtitle_size)

% --- Pitch Parameters (Left Column) ---
titles_pitch = {'Pitch', '', '', '', ''};
ylabels_pitch = {'$\hat{h}$','$\hat{a}_1$','$\hat{a}_2$','$\hat{a}_3$','$\hat{a}_4$'};

for i = 1:5
    nexttile((i-1)*2 + 1)
    hold on
    plot(time,haaaa_hat_p(:,i),'b','LineWidth',line_width)
    xlim([0 10])
    yl = ylim;
    range = yl(2) - yl(1);
    ylim([yl(1) - 0.05*range, yl(2) + 0.05*range])   % expand by ±5%
    ax_local = gca;
    ax_local.YTick = linspace(min(ax_local.YLim), max(ax_local.YLim), 2);  % exactly 2 ticks
    % Uncomment for truth line
    % plot(time,ones(size(time)).*paramsTruth_haaaa(i,1),'b--','LineWidth',1)
    hold off
    if i == 1
        title('\textbf{Pitch}','Interpreter','latex','FontSize',title_size)
    end
    
    grid on
    box on
    set(gca,'FontSize',16,'TickLabelInterpreter','latex')
    ylabel(ylabels_pitch{i},'Interpreter','latex','FontSize',axis_size)
    if i == 5
        xlabel('Time (s)','Interpreter','latex','FontSize',axis_size)
    end
end

% --- Yaw Parameters (Right Column) ---
titles_yaw = {'Yaw', '', '', '', ''};
ylabels_yaw = {'$\hat{h}$','$\hat{a}_1$','$\hat{a}_2$','$\hat{a}_3$',''};

for i = 1:4
    nexttile(i*2)
    hold on
    plot(time,haaa_hat_y(:,i),'r','LineWidth',line_width)
    xlim([0 10])
    yl = ylim;
    range = yl(2) - yl(1);
    ylim([yl(1) - 0.05*range, yl(2) + 0.05*range])   % expand by ±5%
    ...
    ax_local.YTick = linspace(min(ax_local.YLim), max(ax_local.YLim), 2);  % exactly 2 ticks
    % Uncomment for truth line
    % plot(time,ones(size(time)).*paramsTruth_haaaa(i,2),'r--','LineWidth',1)
    hold off
    if i == 1
        title('\textbf{Yaw}','Interpreter','latex','FontSize',title_size)
    end
    grid on
    box on
    set(gca,'FontSize',16,'TickLabelInterpreter','latex')
...
% Apply consistent tick label formatting across all axes
ax = findall(gcf,'Type','axes');
for k = 1:numel(ax)
    ax(k).XTickLabel = compose('%.1f', ax(k).XTick);  % 2 decimal places
    ax(k).YTickLabel = compose('%.5f', ax(k).YTick);
end
% Apply same x-axis limits to all tiles
% ax = findall(gcf,'Type','axes');
% for k = 1:numel(ax)
%     xlim(ax(k), [0 30]);   % Example: limit time axis from 0 to 10 seconds
% end
